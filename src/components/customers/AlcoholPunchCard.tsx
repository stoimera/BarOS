...
interface AlcoholPunchCardProps {
  punchCount: number
  goal: number
  rewarded: boolean
  /** customer = show copy for QR redemption; staff = mark-rewarded flow */
  mode?: "staff" | "customer"
  onAddVisit?: () => void
  onMarkRewarded?: () => void
  className?: string
  // New props for alcohol-specific punch card
  alcoholPunchCount?: number
  alcoholGoal?: number
  alcoholRewarded?: boolean
  onAddAlcoholVisit?: () => void
  onMarkAlcoholRewarded?: () => void
}

export function AlcoholPunchCard({ 
  punchCount, 
  goal, 
  rewarded, 
  mode = "staff",
  ...
  alcoholRewarded,
  onAddAlcoholVisit,
  onMarkAlcoholRewarded
}: AlcoholPunchCardProps) {
  // Use alcohol-specific values if provided, otherwise fall back to general values
  const actualPunchCount = alcoholPunchCount ?? punchCount;
  const actualGoal = alcoholGoal ?? goal;
  const actualRewarded = alcoholRewarded ?? rewarded;
  const actualOnAddVisit = onAddAlcoholVisit ?? onAddVisit;
  const actualOnMarkRewarded = onMarkAlcoholRewarded ?? onMarkRewarded;
    ...
    <Card className={cn(" overflow-hidden", className)}>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base sm:text-lg">
          <span className="text-base sm:text-lg text-blue-500 flex-shrink-0">🍺</span>
          <span className="truncate">Alcohol Punch Card</span>
          {actualRewarded && (
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          {actualOnAddVisit && (
            ...
            >
              ...
              <span className="hidden sm:inline">Add Alcohol Visit</span>
